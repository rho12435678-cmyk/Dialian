...
async def add_review_points_by_bundle(
    guild, member, bundle_type: str = "단품 (1개)"
) -> tuple[int, int]:
    """후기 작성 시 묶음 종류에 따라 포인트를 차등 적립"""
...
async def check_and_add_share_points(guild, member, message) -> bool:
    ...
    async with aiosqlite.connect(DATABASE) as db:
        cursor = await db.execute(
            "SELECT last_share_date FROM user_points WHERE user_id = ?",
            (member.id,),
        )
        row = await cursor.fetchone()
        ...
        await db.execute(
            """
            INSERT INTO user_points (user_id, points, last_share_date) VALUES (?, ?, ?)
...
async def check_and_add_feedback_points(guild, member, message) -> bool:
    ...
    async with aiosqlite.connect(DATABASE) as db:
        cursor = await db.execute(
            "SELECT last_feedback_date, feedback_today_count FROM user_points WHERE user_id = ?",
            (member.id,),
        )
        row = await cursor.fetchone()
